...
class ChatConsumer(WebsocketConsumer):
    ...
    def new_message(self, data):
        author = data['from']
        friend = data['friend']
        author_user = Profile.objects.filter(username=author)[0]
        friend_user = Profile.objects.filter(username=friend)[0]
    ...
    def connect(self):
        ...
        author_user = Profile.objects.filter(username=self.user.username)[0]
        friend_user = Profile.objects.filter(username=self.friend)[0]
        if Room.objects.filter(
            ...
            self.room = Room.objects.create(
                author=author_user, friend=friend_user)
